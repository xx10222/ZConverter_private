...
#define MAXLINE 1024
#define PORTNUM 3600
#define IP "127.0.0.1" //centos server ip(change)

#define HTTP_URL "http://127.0.1.1:8000/" //http directory server ip + port number for http client(change)
#define WEBSOCKET_URL "ws://192.168.1.88:9998" //websocket server ip + port number for websocket client(change)

#define srv_localpath "/home/os2018202001/Desktop/test/" //server local path (location of file) (change)
...
{
	//check hashsum
	write(sockfd, fileName, MAXLINE); //send file name to client
	...
	read(sockfd,buf,MAXLINE); //md5
	strcpy(cli_md5, buf);

	memset(buf, 0x00, MAXLINE);
	read(sockfd,buf,MAXLINE); //sha1
	strcpy(cli_sha1, buf);

	memset(buf, 0x00, MAXLINE);
	read(sockfd,buf,MAXLINE); //sha256
	strcpy(cli_sha256, buf);

	
	printf("@@@@@@@@@@@hash\n%s%s%s",cli_md5,cli_sha1,cli_sha256);
	...
	fgets(srv_md5, sizeof(srv_md5), hfile);
	fgets(srv_sha1, sizeof(srv_sha1), hfile);
	fgets(srv_sha256, sizeof(srv_sha256), hfile);

printf("@@@@@@@@@@@hash\n%s%s%s",srv_md5,srv_sha1,srv_sha256);

	if(strncmp(cli_md5,srv_md5,strlen(srv_md5)))
		printf("send error! - different hashsum!!!\n");
	else
		printf("md5 hashsum ok!\n");

	if(strncmp(cli_sha1,srv_sha1,strlen(srv_sha1)))
		printf("send error! - different hashsum!!!\n");
	else
		printf("sha1 hashsum ok!\n");

	if(strncmp(cli_sha256,srv_sha256,strlen(srv_sha256)))
		printf("send error! - different hashsum!!!\n");
	else
		printf("sha256 hashsum ok!\n");
...
printf("@@@@@@@@@@@hash\n%s%s%s",srv_md5,srv_sha1,srv_sha256);
	...
	strcpy(update_cmd, "./update_file.py ");
